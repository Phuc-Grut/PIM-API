...
{
    public partial class ManufacturerRepository : IManufacturerRepository
    {
        protected readonly SqlCoreContext Db;
        protected readonly DbSet<Manufacturer> DbSet;
        public IUnitOfWork UnitOfWork => Db;
        public ManufacturerRepository(IServiceProvider serviceProvider)
        {
            //var scope = serviceProvider.CreateScope();
            Db = serviceProvider.GetRequiredService<SqlCoreContext>();
            DbSet = Db.Set<Manufacturer>();
        }
        ...
        public async Task<(IEnumerable<Manufacturer>, int)> Filter(string? keyword, IFopRequest request)
        {
        ...
        public async Task<IEnumerable<Manufacturer>> GetListListBox(int? status, string? keyword)
        {
            var query = DbSet.AsQueryable();
            if (!String.IsNullOrEmpty(keyword))
            {
        ...
        public void Update(IEnumerable<Manufacturer> manufacturers)
        {
            DbSet.UpdateRange(manufacturers);
        }
    }
}
