...
{
    public partial class ProductBrandRepository : IProductBrandRepository
    {
        protected readonly SqlCoreContext Db;
        protected readonly DbSet<ProductBrand> DbSet;
        public IUnitOfWork UnitOfWork => Db;
        public ProductBrandRepository(IServiceProvider serviceProvider)
        {
            //var scope = serviceProvider.CreateScope();
            Db = serviceProvider.GetRequiredService<SqlCoreContext>();
            DbSet = Db.Set<ProductBrand>();
        }

        public void Add(ProductBrand productBrand)
        {
            DbSet.Add(productBrand);
        }
        ...
        {
            DbSet.Remove(productBrand);
        }

        public void Update(ProductBrand productBrand)
        {
            DbSet.Update(productBrand);
        }
        ...
        public async Task<(IEnumerable<ProductBrand>, int)> Filter(string? keyword, IFopRequest request)
        {
        ...
        public async Task<IEnumerable<ProductBrand>> GetListListBox(int? status, string? keyword)
        {
            var query = DbSet.AsQueryable();
            if (!String.IsNullOrEmpty(keyword))
            {
        ...
        public void Update(IEnumerable<ProductBrand> productBrands)
        {
            DbSet.UpdateRange(productBrands);
        }
    }
}
