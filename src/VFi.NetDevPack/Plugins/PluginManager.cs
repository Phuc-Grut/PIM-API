...
using VFi.NetDevPack.SharedKernel.Extensions;
...
namespace VFi.NetDevPack.Plugins
{
    ...
    public static class PluginManager
    {
        ...
        public static void Load(IMvcCoreBuilder mvcCoreBuilder, IConfiguration configuration)
        {
            ...
            lock (_synLock)
            {
                ...
                try
                {
                    ...
                    {
                        if (plugin.SupportedVersion != ICVersion.SupportedPluginVersion)
                        {
            ...
            Assembly assembly, string systemName, string filename)
        {
            ...
            {
                Log.Error(ex, "PluginManager");
                throw new InvalidOperationException($"The plugin directory for the {systemName} file exists in a folder outside of the allowed VFi.NetDevPacknode folder hierarchy - exception because of {filename} - exception: {ex.Message}");
            }
        }
